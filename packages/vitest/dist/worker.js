...
import { c as createBirpc } from './vendor-index.ea279c29.js';
import { workerId } from 'tinypool';
import { g as getWorkerState } from './vendor-global.6795f91f.js';
import { s as startViteNode, m as moduleCache, a as mockMap } from './vendor-execute.aa0b364f.js';
...
import './vendor-environments.a0e9418a.js';
import 'node:console';
import 'local-pkg';
import 'node:fs';
import '@vitest/spy';
...
async function run(ctx) {
  ...
  try {
    init(ctx);
    const { run: run2, executor, environment } = await startViteNode(ctx);
    await run2(ctx.files, ctx.config, { ...ctx.environment, environment }, executor);
    await rpcDone();
  } finally {
    inspectorCleanup();
  }
